...
struct Params
{
    ...
    float rest_density = DEFAULT_FLUID_DENSITY; // rho_0 [kg/m³]
...
constexpr Vec3 GRAVITY = {0.f, 0.f, -9.81f}; // m/s²
...
constexpr float MIN_RHO = 1e-6;  // avoid division by zero
...
constexpr int GRID_N_AXIS = 9;
constexpr int GRID_SIZE = GRID_N_AXIS * GRID_N_AXIS * GRID_N_AXIS;
constexpr int GRID_BIAS = GRID_N_AXIS / 2;
constexpr int TABLE_SIZE = GRID_SIZE;
...
constexpr float XSPH_EPS =
    0.75f; // velocity smoothing
constexpr float CONTACT_K =
    800.0f; // penalty spring
constexpr float DIE_CONTACT_MARGIN =
    0.45f * PARTICLE_R; // early contact shell
constexpr float DIE_CONTACT_DAMPING =
    12.0f; // normal damping [N·s/m]
inline const float PARTICLE_AREA =
    PI * PARTICLE_R * PARTICLE_R;  // cross-section [m²]
...
inline bool SIM_GPU_STRICT_SYNC = false;
inline bool SIM_GPU_FUSED_FORCES = false;
inline int SIM_NEIGHBOR_REBUILD_EVERY = 1;
inline int SIM_OUTPUT_EVERY = 1;
