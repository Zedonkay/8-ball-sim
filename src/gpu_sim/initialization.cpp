...
void poisson_disk_sample_sphere(SimState &s)
{
    ...
    // Seed spacing from target count in the effective fluid volume.
...
void inject_ghost_particles(SimState &s, const RigidDie &die)
{
    ...
    for(int k = 0; k < die.n_ghost_offsets && g < N_MAX; ++k)
    {
        ...
        s.vel_x[PING][g] = v_surf.x;
        s.vel_y[PING][g] = v_surf.y;
        s.vel_z[PING][g] = v_surf.z;
        s.vel_x[PONG][g] = v_surf.x;
        s.vel_y[PONG][g] = v_surf.y;
        s.vel_z[PONG][g] = v_surf.z;
